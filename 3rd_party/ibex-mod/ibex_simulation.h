...
//modification for guaranteed integration
#include <QList>
...
public:
	...
	QList<IntervalVector> run_simulation(){
	  QList<IntervalVector> intSolutions;
	  double tn ;
		...
		if (tn < time_T)
		{
		  ...
		  if (delta_t < newh)
			...
			case RK4:
			{
			  ...
			  sol_temp.flush();
			  list_solution_j.push_back(u_j);
			  intSolutions.append(list_solution_j.back().box_jnh_aff->itv());
			  break;
			}
